...
def saparate_worker_class(bbox, enable):
    '''
...
#             print(int(element[-1]))
            ...
            if enable[3]:
                if int(bbox[index][-1]) == 3 : # B
                    to_return.append(bbox[index])
    
            if enable[4]:
                if int(bbox[index][-1]) == 4 : # G
                    to_return.append(bbox[index])
    ...
    return class_worker, np.array(class_object)
...
def PPE_DECISION(boxes,  box_object,  class_names,  enable, visible=True, i_frame=None):
