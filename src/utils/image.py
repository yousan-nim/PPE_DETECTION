...
#              [0,255,0],]
    ...
    # draw the detections
    for box in boxes:
#       x1, y1, x2, y2 = int(float(box[0])), int(float(box[1])), int(float(box[2])*0.70), int(float(box[3])*0.835)
        x1, y1, x2, y2 = int(float(box[0])), int(float(box[1])), int(float(box[2])), int(float(box[3]))
        score = box[-2]
        label = int(box[-2])
#         label = box[-2]
        num = [i for i in range(num_classes) if label==class_names[i]] 
#        print(num)
        clr = colors[label]
#        clr = [0,0,0]
...
#        text = f'{class_names[int(num[0])]}'
        ...
        img = cv2.putText(img, text, (x1 + border, y1 - border), font, font_scale, text_color, text_weight, cv2.LINE_AA)
    return img
#     return [tb_x1, tb_y1, tb_x2, tb_y2, class_names[int(num[0])]]

def draw_detection_origin(
    img,
    boxes,
    class_names,
    # drawing configs
